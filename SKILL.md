...
11. Framework Crosswalk             ← Step 5, only if requested (one subsection per named framework; all eight only in full-crosswalk mode)
...
This skill ships with seven reference files. Load each when its topic enters the analysis. None need to be loaded for a request that is purely about whether the skill applies; load when actually producing the assessment.
